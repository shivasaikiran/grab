...
const AmazonoffersForm = () => {
  ...
  const fetchAds = async () => {
    try {
      const adsSnapshot = await getDocs(collection(db, 'amazonads'));
  ...
  const handleAdSubmit = async (e) => {
    ...
    try {
      ...
      if (adEditId) {
        const adDocRef = doc(db, 'amazonads', adEditId);
        await updateDoc(adDocRef, { imageUrl: adImageUrl });
        toast.success('Ad updated successfully!');
      } else {
        const adsCollectionRef = collection(db, 'amazonads');
  ...
  const handleAdDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'amazonads', id));
  ...
  const fetchBlogs = async () => {
    try {
      const blogsRef = collection(db,'amazonblog');
  ...
  const fetchBanners = async () => {
    try {
      const bannersCollectionRef = collection(db, 'amazonbanner');
  ...
  const handleBannerSubmit = async (e) => {
    ...
    try {
      ...
      if (bannerEditId) {
        ...
        const bannerDocRef = doc(db, 'amazonbanner', bannerEditId);
  ...
  const handleBannerDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'amazonbanner', id));
  ...
  const fetchDeals = async () => {
    const dealsCollectionRef = collection(db, 'amazonoffers');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        const dealDocRef = doc(db, 'amazonoffers', editId);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'amazonoffers', id));
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      if (blogEditId) {
        const blogDocRef = doc(db, 'amazonblog', blogEditId);
        await updateDoc(blogDocRef, { content: blogContent });
        toast.success('Blog updated successfully!');
      } else {
        const blogsCollectionRef = collection(db, 'amazonblog');
  ...
  const handleBlogDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'amazonblog', id));
...
export default AmazonoffersForm;
