...
const FootwarecategoryForm = () => {
  ...
  const fetchBanners = async () => {
    try {
      const bannersCollectionRef = collection(db, 'footwarebanner');
  ...
  const handleBannerSubmit = async (e) => {
    ...
    try {
      ...
      if (bannerEditId) {
        ...
        const bannerDocRef = doc(db, 'footwarebanner', bannerEditId);
  ...
  const handleBannerDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'footwarebanner', id));
  ...
  const fetchDeals = async () => {
    const dealsCollectionRef = collection(db, 'footwareoffers');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        const dealDocRef = doc(db, 'footwareoffers', editId);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'footwareoffers', id));
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      if (blogEditId) {
        const blogDocRef = doc(db, 'footwareblog', blogEditId);
        await updateDoc(blogDocRef, { content: blogContent });
        toast.success('Blog updated successfully!');
      } else {
        const blogsCollectionRef = collection(db, 'footwareblog');
  ...
  const handleBlogDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'footwareblog', id));
...
export default FootwarecategoryForm;
