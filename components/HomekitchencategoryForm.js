...
const HomekitchencategoryForm = () => {
  ...
  const fetchBlogs = async () => {
    try {
      const blogsRef = collection(db,'homekitchenblog');
  ...
  const fetchBanners = async () => {
    try {
      const bannersCollectionRef = collection(db, 'homekitchenbanner');
  ...
  const handleBannerSubmit = async (e) => {
    ...
    try {
      ...
      if (bannerEditId) {
        ...
        const bannerDocRef = doc(db, 'homekitchenbanner', bannerEditId);
  ...
  const handleBannerDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'homekitchenbanner', id));
  ...
  const fetchDeals = async () => {
    const dealsCollectionRef = collection(db, 'homekitchenoffers');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        const dealDocRef = doc(db, 'homekitchenoffers', editId);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'homekitchenoffers', id));
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      if (blogEditId) {
        const blogDocRef = doc(db, 'homekitchenblog', blogEditId);
        await updateDoc(blogDocRef, { content: blogContent });
        toast.success('Blog updated successfully!');
      } else {
        const blogsCollectionRef = collection(db, 'homekitchenblog');
  ...
  const handleBlogDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'homekitchenblog', id));
...
export default HomekitchencategoryForm;
