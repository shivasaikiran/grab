...
const Product = ({ searchTerm }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const querySnapshot = await getDocs(collection(db, 'popularOffers'));
  ...
  return (
    <section className="py-4" id='popularoffers' >
      <div className="px-8 ">
        <div className=" w-[320px]  " >
          <h2 className="text-2xl font-bold text-black sm:text-3xl">
            <span className="relative inline-block mr-2">
            Hot
              <span className="absolute bottom-[-4px] left-0 lg:w-[140px] w-[110px] h-[2px] bg-[#26ca43]"></span>
            </span>
            Deals
          </h2>
        </div>
