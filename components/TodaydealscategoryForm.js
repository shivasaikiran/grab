...
const TodaydealscategoryForm = () => {
  ...
  const fetchBanners = async () => {
    try {
      const bannersCollectionRef = collection(db, 'todaydealbanner');
  ...
  const handleBannerSubmit = async (e) => {
    ...
    try {
      ...
      if (bannerEditId) {
        ...
        const bannerDocRef = doc(db, 'todaydealbanner', bannerEditId);
  ...
  const handleBannerDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'todaydealbanner', id));
  ...
  const fetchDeals = async () => {
    const dealsCollectionRef = collection(db, 'todaydealoffers');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        const dealDocRef = doc(db, 'todaydealoffers', editId);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'todaydealoffers', id));
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      if (blogEditId) {
        const blogDocRef = doc(db, 'todaydealblog', blogEditId);
        await updateDoc(blogDocRef, { content: blogContent });
        toast.success('Blog updated successfully!');
      } else {
        const blogsCollectionRef = collection(db, 'todaydealblog');
  ...
  const handleBlogDelete = async (id) => {
    try {
      await deleteDoc(doc(db, 'todaydealblog', id));
...
export default TodaydealscategoryForm;
