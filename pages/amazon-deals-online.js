...
const amazonoffers = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const querySnapshot = await getDocs(collection(db, 'amazonoffers'));
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const bannersSnapshot = await getDocs(collection(db, 'amazonbanner'));
      ...
      />
        ...
        <div className="p-1   relative mb-1 bg-green-500 lg:mb-0 lg:mr-2 w-full lg:w-[30%] h-[90px] lg:h-[130px] flex lg:flex-col flex-row  lg:space-y-2">
          <span className="relative inline-block mb-1 mr-2 lg:mb-2 lg:top-0 top-7 ">
            <h1 className="font-bold text-white lg:text-center text-start lg:text-4xl">Amazon Deals  </h1>
