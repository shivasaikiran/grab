...
const Todaydeals = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const querySnapshot = await getDocs(collection(db, 'electronicsoffers'));
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const bannersSnapshot = await getDocs(collection(db, 'electronicsbanner'));
  ...
  const fetchBlogs = async () => {
    try {
      const blogsRef = collection(db, 'electronicsblog',);
      ...
      />
        ...
        <div className="p-1 hidden   relative mb-1 bg-green-500 lg:mb-0 lg:mr-2 w-full lg:w-[30%] h-[90px] lg:h-[130px] lg:flex lg:flex-col flex-row  lg:space-y-2">
          <span className="relative inline-block mb-1 mr-2 lg:mb-2 lg:top-0 top-7 ">
            <h1 className="font-bold text-white lg:text-center text-start lg:text-4xl">Electronics   </h1>
            <span className="absolute hidden sm:block bottom-[-4px] left-24 lg:left-24 lg:w-[190px] w-[160px] h-[2px] bg-white"></span>
          </span>
    ...
    <FaFacebook size={30} className="text-blue-500 transition-transform duration-300 hover:scale-125" />
      ...
      <div className="mb-4 w-[350px] lg:hidden md:hidden">
          <h2 className="text-2xl font-bold text-black sm:text-3xl">
            ...
            Electronics
              <span className="absolute bottom-[-4px] left-0 lg:w-[155px] w-[120px] h-[2px] bg-[#26ca43]"></span>
            </span>
          </h2>
        </div>
