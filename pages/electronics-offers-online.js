...
function Electronics() {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'electronicsbanners'));
  ...
  useEffect(() => {
    const fetchMobileItems = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'categories', 'electronics', 'items'));
      ...
      />
        ...
        <div className="p-1 hidden   relative mb-1 bg-green-500 lg:mb-0 lg:mr-2 w-full lg:w-[30%] h-[90px] lg:h-[130px] lg:flex lg:flex-col flex-row space-x-4 lg:space-y-2">
          <span className="relative inline-block mb-1 mr-2 lg:mb-2 lg:top-0 top-7 ">
            <h1 className="font-bold text-white text-start lg:text-center lg:text-4xl">Electronics  </h1>
