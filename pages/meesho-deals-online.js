...
const meeshooffers = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const querySnapshot = await getDocs(collection(db, 'meeshooffers'));
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const bannersSnapshot = await getDocs(collection(db, 'meeshobanner'));
...
const [showMore, setShowMore] = useState(false);
      ...
      />
        ...
        <div className="p-1   relative mb-1 bg-green-500 lg:mb-0 lg:mr-2 w-full lg:w-[30%] h-[90px] lg:h-[130px] flex lg:flex-col flex-row  lg:space-y-2">
          <span className="relative inline-block mb-1 mr-2 lg:mb-2 lg:top-0 top-7 ">
            <h1 className="font-bold text-white lg:text-center text-start lg:text-4xl">Meesho Deals  </h1>
  ...
  {banners.map((banner) => (
    <div key={banner.id} className="relative w-full h-full overflow-hidden">
      ...
      <Product3/>
      <Product1/> */}
      <div className="grid justify-center grid-cols-3 px-2 mt-8 mb-10 lg:px-8 sm:grid-cols-3 ">
