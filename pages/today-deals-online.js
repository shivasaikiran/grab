...
const Todaydeals = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const querySnapshot = await getDocs(collection(db, 'todaydealoffers'));
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const bannersSnapshot = await getDocs(collection(db, 'todaydealbanner'));
  ...
  const fetchBlogs = async () => {
    try {
      const blogsRef = collection(db, 'todaydealblog',);
  ...
  const staticPaths = [
    '/flipkart-offers-online',
    '/myntra-offers-online',
    '/amazon-offers-online'
  ];
...
const [showMore, setShowMore] = useState(false);
      ...
      />
        ...
        <div className="p-1   relative mb-1 bg-green-500 lg:mb-0 lg:mr-2 w-full lg:w-[30%] h-[90px] lg:h-[130px] flex lg:flex-col flex-row  lg:space-y-2">
          <span className="relative inline-block mb-1 mr-2 lg:mb-2 lg:top-0 top-7 ">
            <h1 className="font-bold text-white lg:text-center text-start lg:text-4xl">Today deals  </h1>
  ...
  <span className='ml-2'><FaArrowRight/></span>
      ...
      <FaFacebook size={40} className="text-blue-500 hover:scale-125 mobile-size" />
        ...
        <div className="flex flex-col w-full hidden sm:block md:block  text-center bg-green-500 lg:w-2/3  h-[90px] lg:h-[130px] lg:ml-2 lg:mt-0">
  {banners.map((banner) => (
    <div key={banner.id} className="relative w-full h-full overflow-hidden">
        <img 
        src={banner.bannerImageUrl} 
      ...
      {/* Banners Section */}
      {/* <Product2 /> */}
      <div className="px-1 lg:hidden md:hidden">
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-6 lg:grid-cols-6">
      ...
      <Link href='/Electronics-deals-online'><div className="flex justify-center">
        ...
        <Link href='/Footware-deals-online'><div className="flex justify-center">
          <div className="flex items-center justify-center p-2  border rounded-md shadow-md border-gray-300 hover:border-green-500 hover:shadow-green-500 h-[60px] w-[100px] lg:h-[70px] lg:w-[420px]">
          <p className='text-xs font-bold lg:text-base hover:text-green-500'>Footware</p>
          </div>
        </div></Link>
        ...
        <Link href='/Health-fitness-deals-online'><div className="flex justify-center">
          <div className="flex items-center justify-center p-2  border rounded-md shadow-md border-gray-300 hover:border-green-500 hover:shadow-green-500 h-[60px] w-[100px] lg:h-[70px] lg:w-[420px]">
